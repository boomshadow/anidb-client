...
| Domain | Spec |
|--------|------|
| Object layer & attribute resolution — `Anime`/`Episode`/`File`/`Group` construction from a title, id or path; lazy resolution through in-memory value → cache → network, with falsy cached values counting as answers; unknown attributes answering `None` rather than raising, while a name a class declares itself is answered by that declaration alone and never stood in for by the cached row; while a fetch that cannot be answered at all raises the reason — banned, timed out, refused — on a bounded wait rather than blocking; illegal objects for ids AniDB does not recognise (and the rule that becoming illegal must never block the waiter); `update()` as the forced refresh; how one anime's relations resolve; equality/containment semantics; cover-image download | [SPEC-001](docs/specs/SPEC-001-object-layer-and-attribute-resolution.md) |
| The relation graph — AniDB as the authority on what belongs to a show and the library's refusal to decide it, so relation types are returned as data and never applied as policy; `relations` as `(relation_type, Anime)` pairs over AniDB's own vocabulary, with `character` an anime-to-anime link and never a character record; the transitive walk over a cyclic graph, the type reported being the edge an anime was first reached by rather than the route; the mechanical bounds — a traversal budget on by default and an optional depth — against the caller's own optional policy of relation types, mylist membership and `exclude` walls; a truncated answer that names the bound that ended it, including a depth reached and an id AniDB does not recognise, because an incomplete answer shaped like a complete one is the failure this exists to prevent; the ambiguity of `other`, which carries both a franchise ancestor and the show's own entries, and the measured graph the budget exists to survive | [SPEC-010](docs/specs/SPEC-010-the-relation-graph.md) |
| UDP session & rate limiting — session establishment with the registered client identity and optional AES-encrypted sessions (a configured key makes encryption mandatory); a handshake whose success is recognised by a whitelist and which always settles, with refused credentials latched and never re-sent while a ban is retried after a back-off, and the reported NAT address treated as advisory rather than as something that can fail the login; sender/listener thread pair, the order they start in, and the state they share through locked accessors (pending commands, session key, cipher, pacing counters), with the pacing lock never held across a back-off sleep; a sender that waits to be woken rather than polling; a callback thread per reply and why it is not a pool; monotonic timing for every interval measured; tag-based correlation over an unordered datagram protocol, with every reply classified from the code table *before* its tag is consulted so a refusal closes the gate untagged, correctly tagged or wrongly tagged, and still settles the command it answered; silence — AniDB's own way of enforcing a ban — as a first-class ban signal that outranks the retry budget; a refusal that carries its cause, which kind of back-off it is and its unrounded remaining time as data rather than only as a message; a read-only health surface, including which kind of back-off stands and the address AniDB reported back, so an embedder can tell a quiet client from a gated one -- and a banned one from a busy upstream -- without sending anything, handed out by a public accessor so that surface is reachable without a private attribute -- for reading, not as a way to send, with a hand-built command explicitly not a supported entry point; a source address that is one stable identity, the outgoing port pinned and never shared so a duplicate bind fails loudly instead of splitting one client's replies; outbound pacing (short burst then steady rate, application time counted toward the interval, burst allowance refreshed after idle) in a limiter the embedder may supply, which buys lifetime rather than latitude; two exponential jittered back-off schedules sharing a ceiling and chosen by the response code's disposition -- half an hour for a banned client, half a minute for an upstream that is merely busy, with a change of kind restarting the doubling -- cleared by a successful auth, held as a window during which nothing is sent rather than as a sleep -- so the listener keeps reading and a caller is told rather than held; session-loss detection and re-auth with the discovering command re-queued at the front; command timeout with an attempt budget that is spent rather than renewed (auth and encrypt back off instead); every request carrying an outcome — the reply, or the reason there will not be one — settled after its callback has run, and a transport that fails every caller it was working for rather than stopping silently; NAT keepalive and idle session refresh; a stop that ends both threads and gives the pinned port back before it returns, because a thread blocked in a receive call holds the port after the socket is closed, so the loop is woken by a datagram addressed to itself and never to AniDB; a teardown that sends nothing when there is no session to end and fails what was still in flight; the two containment rules — a library never terminates its host process, and a waiter is always released | [SPEC-002](docs/specs/SPEC-002-udp-session-and-rate-limiting.md) |
| Caching & the local data model — the SQL cache (SQLite/PostgreSQL/MySQL) and the on-disk XML caches; the freshness policy (hard one-day floor, freshness roll at most once per 20 hours so a daily cron still gets a decision, age-weighted probability starting at nothing in week one, anime-specific bonus decaying from AniDB's own last-change proximity); 36-hour XML refresh with a size sanity check and fall-back-to-stale on a failed fetch, a download that arrives and fails that check falling back the same way because from the caller's side the two are one situation, at a level that states the outcome — a warning naming how stale the answer has become when there is a copy to carry on with, an error only when there is not; what the schema guarantees (dual timestamps, relations as reconciled rows, database-level enum vocabularies defined once and selected from by the wire layer rather than restated, wide-integer SQLite variant, generic file rows, nullability stated on every column rather than inferred from its annotation); the block-scoped lifetime of a cache session, so a failure or an early return still returns the connection; the engine handed back alongside the session factory, because the engine is what owns the pool and `close()` is what disposes it; how the engine is opened for a threaded process (WAL attempted with the obtained mode read back and logged, an explicitly chosen busy timeout, foreign keys enforced on every SQLite connection, a bounded connection pool whose size `init()` exposes, and pragmas that fire for SQLite alone); best-effort cache writes; file cache identity re-validated by size then size+ed2k; **no migration story — recreate the database after upgrading**, which is also what makes enforcing foreign keys safe, and the per-backend DDL snapshot that makes an unintended schema change a build failure rather than a cache rebuild for every user | [SPEC-003](docs/specs/SPEC-003-caching-and-the-local-data-model.md) |
| File identification & mylist — size+ed2k as the identity AniDB recognises, hash reuse gated on size and mtime, ed2k degenerate cases; generic entries when AniDB has never seen the file; two-part inference (anime from parent directory at the normal threshold then from a stripped filename at a deliberately lower one; episode from an ordered regex ladder whose fallbacks are gated behind the single-episode test); AniDB's special/opening/ending/trailer numbering with the halfway-through-credits ending guess; multi-episode ranges (filename parsing supports them, the file API does not) and part files; mylist add/edit via one entry point, the one-entry-per-episode rule, per-episode adds for generic files, local-first edits vs. add-then-read, a write the transport cannot deliver raising rather than reporting a change that never happened; removal clearing cached state regardless of reply; the file-less add from an anime id and an episode number as its own entry point, because the rule above reaches its state by removing -- an add that sends no edit and so cannot overwrite, one request with no probe and no read after it, three outcomes rather than an exception on AniDB's no, an episode number that must name exactly one episode because the command reads zero as all of them, and no cached row because AniDB returns no identifier to write one against; promotion of a generic entry once AniDB learns the real file | [SPEC-004](docs/specs/SPEC-004-file-identification-and-mylist.md) |
...
| Configuration & credentials — `init()` as the single entry point and the database URL as its only required argument, with `connect()` beside it as the optional third lifecycle call that opens the lazily-established session at startup instead of on the first request -- idempotent, bounded by the caller's own patience rather than only the protocol's (a different quantity from the per-command timeout that drives the sweep and the retry budget, and never to be conflated with it, with an abandoned wait leaving the handshake to settle for the next caller to join), no extra traffic in the ordinary case, and deliberately not a pollable health check, because here a `ping()` is metered by something that bans; with an in-memory SQLite URL refused outside `db_only` and the cache pool's size exposed as an argument; three credential arrangements (direct, netrc, or `db_only` which must not demand them); exact netrc machine-name sets for AniDB credentials, database credentials (hostname only — no port, no IPv6 brackets, case-insensitive) and the fanart key; database password injection only when the URL has none, only when the credential belongs to the URL's user, rebuilt structurally with percent-encoding rather than string surgery; registered client identity deliberately unrelated to the distribution version; encryption key; a pinned outgoing UDP source port rather than a fresh one per call, because AniDB meters the address a datagram arrives from; a replaceable rate limiter so the pacing and ban state can outlive the process, seeded with durations rather than instants because a monotonic deadline does not survive the process that produced it, and refusing incoherent state rather than repairing it; logging setup with AUTH contents never logged; the per-socket-operation HTTP timeout; a second `init()` refused rather than ignored or rebuilt; an all-or-nothing construction that acquires the cache before the transport so a failure leaves nothing bound; `close()` as the shutdown that gives back the transport, the connection pool, the fanart key and the pinned source port, and that leaves the library able to start again | [SPEC-006](docs/specs/SPEC-006-configuration-and-credentials.md) |
...
| CI pipeline & security scanning — six stages (validate/test/build/security/drift-detection/publish); lockfile and spec/ADR INDEX freshness gates; lint/typecheck/test against a real PostgreSQL service; wheel+sdist built once and carried to publish; shared `ci-templates` Semgrep/Grype/exception-audit consumed from their default branch with a daily rescan schedule; **no container scanning** (the deliverable is a wheel, the image is a harness); MR-only, no-Renovate `anchor-watch` drift gate that does not allow failure; conventional-commit grammar checked on MR titles and on the subjects reaching `main`; tag-only PyPI publish over OIDC trusted publishing, gated behind validate, test and build, with the security stage empty on a tag because the scanners run on merge requests, the default branch and the schedule instead | [SPEC-008](docs/specs/SPEC-008-ci-pipeline-and-security-scanning.md) |
| Release & publication — maturity stated to the index in the package metadata rather than derived from the version; the version declared once in the package in SemVer and normalised to PEP 440 only in the built artifact; the release tag grammar, admitting `alpha`/`beta`/`rc` pre-releases alone and refusing build metadata, with a refusal that names the spelling that would have worked; the permanence of tags and published versions and the fix-forward rule that follows from it; a gate applied on both sides of the tag that checks legality and that the artifacts carry the version implied, and that fails loudly on a malformed tag rather than matching no job at all; upload over OIDC trusted publishing with no stored credential and the four claims PyPI actually verifies; release notes carried on the release pages rather than in a changelog file, half generated from the commits and half written, with merge commits as the only thing filtered out, and scoped back to the last final release rather than the last tag so pre-release notes are cumulative and a final announces the whole body of work; the conventional-commit grammar checked on both sides of the squash and the accepted type vocabulary held against the routing table in both directions, so a type nothing routes cannot fall silently into the catch-all; a release cut locally behind preflight conditions that read the default branch's newest *push* pipeline rather than a security schedule that runs no tests, with the version bump verified before it is pushed and the build cache keyed on the version file so the bump is not silently cached away; what a pre-release means to a consumer | [SPEC-009](docs/specs/SPEC-009-release-and-publication.md) |
...
This project keeps no `CHANGELOG.md` — the GitHub and GitLab release pages are the
changelog (ADR-003), and their contents are generated from the commit subjects on
`main`. Squash-on-merge makes a merge request's **title** its squashed subject, so an MR
title is user-facing copy that ships.

Two consequences, both easy to get wrong:

- **The Conventional Commits type is a routing decision, not decoration.** It picks the
  heading the change appears under: `feat` → Added, `fix` → Fixed,
  `perf`/`refactor`/`revert` → Changed, `docs` → Documentation,
  `test`/`ci`/`build`/`chore`/`style` → Internal. Anything unparsable lands in Other. A
  `feat:` on an MR whose headline change is a bug fix files that fix under "Added".
- **One MR is one line.** If an MR needs two headline sentences to describe it, it is two
  MRs — squash-merge collapses it to a single line of history forever, so bundling two
  notable changes permanently hides one of them.

`cliff.toml` is the line of truth for that mapping, and CI enforces the grammar on both
sides of the squash — the MR title on every merge request, and the subjects landing on
`main` on every push. Check a title before opening the MR with
`task check:title -- "feat: add a thing"`, and preview what a release would say with
`task publish:notes -- v0.0.1`.

What no gate can check is whether a well-formed title is *accurate*. That part is on you.
...
src/anidb_client/
  ...
  animeobjs.py      Anime/Episode/File/Group + lazy resolution → SPEC-001/SPEC-004/SPEC-010
  ...
  errors.py         Exception hierarchy, ban cause, back-off kind → SPEC-002
scripts/            Repository tooling, not packaged
  release_tag.py    The release-tag gate                      → SPEC-009
  conventional_commit.py  The commit/MR-title gate            → SPEC-008/SPEC-009
