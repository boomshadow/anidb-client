...
- **Tests:** external mapping — seasons, per-episode maps, special numbering and movie parts — in `tests/unit/test_external_mapping.py`; the field converters and bitmask machinery shared with the UDP layer in `tests/unit/test_mapper.py`; enum conversion of unknown codes in `tests/unit/test_enum_converters.py`, which also holds the checks that the conversion tables select from the schema's vocabularies (SPEC-003) and leave none of them unreachable. The HTTP fetches are bounded and covered in `tests/unit/test_http_timeouts.py`, and the fanart failure semantics above — not-found, rate-limited with its capped back-off, and the other failures, both HTTP-status and transport-level, that end the lookup while keeping what was already gathered — in `tests/unit/test_fanart_http.py`. The parse loop that builds the mapping table from the anime-list document is exercised directly in `tests/unit/test_external_mapping.py`, because the suite's own fixture builds that table itself and would otherwise leave the production loop uncovered.
