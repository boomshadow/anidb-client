...
description: "Behavioral expectations for initialising anidb-client: init() as the single required entry point, the database URL and the in-memory URL it refuses outside db_only mode, the connection-pool size it exposes, the three credential sources it resolves (direct arguments, a netrc file, or neither in db_only mode), the exact netrc machine-name matching rules for AniDB credentials, database credentials and the fanart key, safe injection of a netrc-sourced password into the SQL URL, the registered client identity, the encryption key, the pinned outgoing UDP port, the replaceable rate limiter whose resumable state — the back-off window, how far it has doubled, how it arose and which kind of back-off it is — is stated as durations rather than instants so it can outlive the process, logging setup — credentials suppressed and the session key AniDB returns replaced by a salted per-process fingerprint wherever a payload is written, including the successful-login line at info and the two payload paths at warning and error rather than debug, and including a logger that exists from import, so no path reachable before init() can fail by reporting something too early — the refusal of a second init() while one is already live, the all-or-nothing construction that leaves nothing running when init() fails, connect() as the optional third lifecycle call that establishes the lazily-opened session at startup rather than on the first request — idempotent, bounded by the caller's own patience rather than only by the protocol's, adding no traffic in the ordinary case, and deliberately not a health check — and close() as the clean shutdown that gives back everything init() took, including the pinned source port."
status: accepted
tags: [configuration, init, credentials, netrc, database-url, sql-url, db-only, in-memory, connection-pool, client-registration, client-name, client-version, encryption-key, api-key, fanart-key, logging, logger, log-redaction, session-key, bearer-token, fingerprint, pre-init, udp-port, source-port, port-pinning, rate-limiter, pacing, ban-state, back-off-kind, durability, injection, http-timeout, close, shutdown, connect, eager-connect, fail-fast, startup, readiness, lifecycle, re-entry, idempotence, teardown, resource-management, engine-disposal]
---
...
**What may be resumed is stated as durations, never as instants.** The back-off deadline is held on a monotonic clock, whose zero point is undefined and process-local, so a deadline stored by one process means nothing in the next — and means nothing *silently*, reading as already elapsed or as hours away with no error either way. So a limiter is told how many seconds of back-off remain and how long ago the last command went out, and converts those to its own clock. Alongside those it is told how far the back-off has doubled, how it arose, and **which kind of back-off it is** — a ban, or an upstream that was merely busy. The kind has to resume with the rest of it or a restart silently converts one into the other, and the process that comes back reports a ban to the same status endpoint that was reporting a busy upstream a moment earlier. Where an application names no kind, the back-off resumes as a ban: the window is whatever it was told either way, only the next escalation differs, and the mistake that default can make is waiting too long rather than sending into a service that has banned this client. Every field that can be seeded has a public reader, so state that can be resumed can also be captured; a limiter that could rehydrate something it had no way to store again would be a worse trap than one that could not resume at all.
...
**Logging** — a caller may supply its own logger, which is used as-is. Absent one, the library configures a logger at the requested level, attaches a syslog handler, and in debug mode additionally logs to standard error. **Nothing that authenticates as the account reaches the log.** The AUTH command's contents are suppressed even at debug level, where every other command is logged in full — and so is the session key AniDB answers it with, which is replaced by a stable fingerprint wherever it would otherwise appear.

That second half was missing, and the sentence here used to claim it anyway. A session key is a bearer credential: it authenticates as the account until it expires or is logged out, so it sits on the same side of this line as the password. It reached the log by every route it had: inbound in the login reply that delivers it, outbound as the parameter every command after the handshake carries, and — the one that matters most — in the line announcing a successful login, which is at **info**. So this was never something only a debug session could expose. A client that authenticated wrote a live bearer credential into its log by default, and two of the payload paths are at warning and error besides.

**A fingerprint rather than a blackout**, because the two are not equally costly. What debugging needs from these lines is whether two of them belong to the same session and whether a re-authentication produced a new one; both survive a fingerprint, and only the ability to *use* the key is removed. Dropping the payload instead would protect one field by discarding the packet around it, and most of this transport's behaviour is observable nowhere else. The fingerprint is salted per process, so it correlates within a run and means nothing outside one. The address AniDB reports back is deliberately left intact beside it: it is the only outside confirmation of which source address is being metered (SPEC-002), and it is not a secret.
...
`init()` sends nothing. The transport is started but the handshake is lazy: nothing reaches AniDB until something first needs a session, so a wrong password or a standing ban is discovered by whichever request happens to be first. For a script that is the right trade. For a long-running service it is the difference between a process that refuses to start and one that starts, reports itself healthy, and fails in front of a user an hour later.

**`connect()` asks the question at startup instead.** It establishes the session, or raises the reason it cannot — the refusal AniDB gave, the back-off that forbade sending, or a timeout — on a bounded wait. It is the third of the three lifecycle calls and the only optional one; a caller content to find out later never needs it.

**It is idempotent.** A session already up is left alone, and a handshake already in flight is waited on rather than duplicated. That is a deliberate property rather than a convenience: against an API that meters by request frequency, a call which logs in again on every invocation is a way to earn a ban, and this is exactly the shape someone would reach for. It is therefore also the opposite of re-authentication, which drops a live session on purpose because a *lost* session needs that and a startup check must not do it.

**It adds no traffic in the ordinary case**, because the handshake happens either way; it moves it from the first request to startup. Only a client that connects and then asks for nothing pays for a login it did not otherwise need.

**The caller may bound its own wait.** Absent a bound, the wait is the transport's handshake budget — sixty seconds as shipped, which is longer than most orchestrators will hold a container's start open, and which nothing an embedding application could reach used to change. A service whose own startup budget is shorter than that had no way to say so, and an operator setting one in their own configuration would have been setting a value that did nothing.

What the bound governs is **how long the caller waits, not how long the protocol gets**, and those are different claims. Giving up early is safe by construction rather than by convention: the handshake is not cancelled, it settles on its own, and the next call joins whatever it settled as rather than starting a second one. So the refusal names a wait that ended rather than a handshake that failed, because only one of those is known at that moment. A bound of zero is therefore coherent — *tell me now* — and a negative one is refused.

This is deliberately not the per-command reply timeout. That value also drives the listener's socket, the timeout sweep and therefore the retry budget (SPEC-002), so lowering it to bound a startup check would change how every command behaves for the life of the process. Wanting a five-second startup is not wanting fewer retries. `close()` takes the same kind of argument at the other end of the process, for the same reason.

**It is not a health check and must not be polled.** Other clients make this a `ping()`, and there it is free; here every command is metered by a service whose enforcement is an IP ban, so the same habit on a readiness probe's timer earns one. The repeated question is answered by the transport's health surface (SPEC-002), which reads state already held and sends nothing. A cache-only client has no session to open, and asking says so rather than doing nothing quietly.
...
- **Tests:** credential resolution and the `db_only` path in `tests/unit/test_init_credentials.py`; the in-memory refusal — including that no UDP link is opened before it — and the pool-size argument reaching the engine in `tests/unit/test_init_database.py`; the SQL URL rewriting rules — hostname matching, user pairing, percent-encoding, IPv6 and ports — in `tests/unit/test_sql_url_credentials.py`; the outgoing UDP port — that the default is the pinned one, that it does not move between calls, and that a caller may still choose its own — in `tests/unit/test_init_udp_port.py`; the rate limiter this call accepts in `TestTheLimiterInitIsGiven` in `tests/unit/test_ratelimit.py`, which pins that the limiter given is the one handed to the transport, that omitting it leaves the transport to build its own, that a resumed one arrives still banned and still carrying the kind of back-off it was resumed with, and that it and the ban vocabulary on both axes are named in the declared public surface; the HTTP timeout's presence at every call site in `tests/unit/test_http_timeouts.py`; that no session key reaches the log — as it arrives, at the default level on the login line, on the commands that carry it, and through the fingerprint that still lets two lines be recognised as one session — in `TestTheSessionKeyNeverReachesTheLog` in `tests/integration/test_link.py`, which also pins that the reported address beside it is *not* redacted and that the older credential suppression still holds; that a logger exists before `init()` and that the entry points reachable that early report rather than raising, in `tests/unit/test_xml_cache_fetch.py`; the package's declared public surface in `tests/unit/test_package.py`. `TestConnectingAtStartup` in `tests/unit/test_lifecycle.py` covers the third lifecycle call: that it is declared, that it reaches the transport, that a caller's bound reaches it too while omitting one keeps the default, that the reason it could not connect reaches the caller rather than being swallowed, and that a cache-only client, an uninitialised one and a closed one each say which they are. The transport half is `TestConnectingOnDemand` in `tests/integration/test_link.py`, which pins the properties that make it safe to offer: that it authenticates once, that calling it repeatedly does not authenticate again, that it is not re-authentication — which does log in again, on purpose — that a refused credential and a latched refusal are raised without re-sending, that a standing ban is raised with nothing reaching the wire, and that an unanswered handshake fails rather than hanging a caller's startup. The lifecycle is covered in `tests/unit/test_lifecycle.py`: that a second `init()` is refused, that the refusal names the way out and leaves the first client working, and that closing first makes a second call legal; that a failed `init()` builds no transport when the cache is what refused, disposes the cache when the transport is what refused, leaves no global set and no fanart key behind, and lets a corrected call succeed; that `close()` disposes the engine, drops the session factory, clears the key, stops the transport, refuses to hand a stopped one out, and still returns the pool when stopping raises; and that closing twice or without having initialised is a no-op. The reported failure is pinned there end to end with a real socket, in `TestThePinnedPortComesBack`: after `close()` the same port binds again, immediately and in the same process.
