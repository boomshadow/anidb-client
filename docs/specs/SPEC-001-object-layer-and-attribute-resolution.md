...
That order is the **fall-through**, and it answers only for names the classes do not define themselves. Where a class declares an attribute of its own, that declaration is the answer, and the fall-through must never stand in for it — not even when the declared one fails. The two need not agree in shape: `Anime.relations` answers `(relation_type, Anime)` pairs while the cached row's own relations are database rows, so a substitution returns something of the wrong type under the right name, and the error surfaces in the caller rather than at the fault. A declared attribute that fails says so; it does not quietly become the cached row.
...
`Anime.relations` is a list of `(relation_type, Anime)` pairs — sequels, prequels, side stories and the rest — built from the relation rows cached alongside the anime. An anime the cache has never seen is resolved first, by the ordinary fetch every other attribute read performs, and answers its relations from what comes back. This is the normal condition during a walk rather than an unusual one: the anime a caller names has a cached row, while the anime the walk reaches *through* it may be ones nobody has ever asked about.

The graph those pairs form, and `Anime.related_anime()` which walks it, are **SPEC-010's** subject: what the walk reports, what bounds it, and why the library returns AniDB's answer rather than deciding what a show is. What belongs here is only how a single anime's relations resolve, which is the ordinary attribute resolution above.
