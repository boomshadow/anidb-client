...
**The test job retries; nothing else does.** A tag cannot be re-pointed, so a test that fails for a reason unrelated to the code does not cost a rerun — it costs a version number. That happened: a handshake test lost a race on the `v3.0.0` tag pipeline, which skipped the build and the upload. The job is retried on a script failure and on the two infrastructure failures that are not statements about the code at all.

Retrying re-runs genuine failures as well, and that is accepted rather than worked around: a real break fails every attempt and is still reported, only later. The cost is pipeline time.

**`anchor-watch` is deliberately excluded.** Its findings are the output of the job rather than noise in it, and a retry would re-roll a nondeterministic reviewer until it stopped objecting — which is the opposite of what that gate is for.
...
It also asserts that each one carries the PEP 561 marker that makes this package's annotations readable downstream (SPEC-007). That check belongs here rather than in the test suite for a reason worth stating: the development environment installs this project editable, so the import system resolves the package back to the working tree, and a test asserting the marker exists would pass just as happily on a build that shipped nothing. The build stage is the first point at which the artifact is a real thing that can be looked inside.
...
- **Related specs:** SPEC-000 (the framework anchor-watch enforces, and the index-generation requirement the validate stage checks); SPEC-007 (the same lint, type, spell and test checks as they run locally, the supply-chain posture the scanning backs, and the typed-distribution claim whose artifact half this pipeline is the only place that can check); SPEC-003 (why a real PostgreSQL service is required rather than SQLite); SPEC-009 (what the publish stage actually verifies before it uploads, and the tag rules it enforces).
