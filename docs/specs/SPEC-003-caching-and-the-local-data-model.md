...
**A download that arrives unusable is a failed fetch.** A document that does not verify falls back exactly as an unreachable server does, because from the caller's side the two are one situation: the copy from last time is still on disk, still the last thing that passed the check, and still a better answer than an error. Falling back is not trusting the bad download — that is discarded, and what stays in use is the copy that already met the same bar. This is why the reasoning here is not the reasoning that governs a lockfile hash: the check is a sanity check against truncation, not an integrity boundary.

**The log level states the outcome, not the event.** A refresh that fell back to the copy on disk is a warning — nothing the caller asked for has failed, and it is about to get an answer. A refresh with nothing to fall back on is an error, because the caller is about to be told so instead. The warning names how old the copy it is carrying on with has become, which is the part an operator can act on: refreshes that keep failing are survived indefinitely, there being no ceiling on staleness anywhere in this policy, so the age in that line is the only place the drift is visible.

**A download that is not moved into place is removed.** Every way out of the fetch other than the rename discards its temporary file, whether the download failed or arrived and did not verify. The temporary name is unique per attempt, so one left behind is never reused — it is simply never cleaned up either, and they accumulate one per failure in a directory the user did not choose. Removal is best-effort: a cleanup that cannot delete the file says so and carries on, because failing there would replace the fetch failure being reported with a less informative one.
