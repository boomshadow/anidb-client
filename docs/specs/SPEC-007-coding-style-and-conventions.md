...
**The distribution says it is typed.** Strictness that stops at the packaging boundary is strictness nobody downstream can use. Under PEP 561 an annotated package that ships no marker *is* an untyped package: a consumer's type checker refuses the import outright, however completely the code is annotated, and pushes them to one of two waivers — one that replaces the whole library with `Any` and discards every annotation here, and one that reads them but leaves a documented hole in strict typing at exactly the boundary a wrapping application most wants checked. So the package carries the marker, and the metadata carries the matching classifier. They are two halves of one claim and are held together by a test, because removing either alone leaves the distribution describing itself two different ways.

What that claim does *not* extend to is the object layer's implicit attributes. `Anime`, `Episode`, `File` and `Group` answer most reads through a fall-through that cannot be typed more precisely than the cached row it forwards to (SPEC-001), so those stay `Any` for a consumer exactly as they are here. The marker makes the declared surface checkable — `init()`, the errors, the enumerations, the methods and properties each class states for itself. It does not make the dynamic half checkable, and saying otherwise would promise a consumer something this library cannot deliver.

The marker's presence in the *artifact* is checked where the artifact exists, in the build stage (SPEC-008), and not in the test suite. The development environment installs this project editable, so the import system resolves the package back to the working tree whether or not packaging would have shipped anything — a test there would pass on a build that ships nothing.
...
**A test waits for the condition it is asserting, not for a proxy for it.** Where a result is assembled by more than one thread, waiting for the first piece to appear and then asserting on the second is a race rather than a check — the two are written under separate acquisitions of the same lock, so there is a window in which the first is true and the second is not, and it widens on a loaded machine. Wait for the whole condition. The rule is worth stating because the failure it produces is the expensive kind: it passes locally, and it fails somewhere that costs something.
...
- **Related specs:** SPEC-008 (the CI pipeline that runs these same checks, the scanning that reads `.grype.yaml`, and the build-stage assertion that the typed marker reached the wheel and the sdist); SPEC-003 (why the PostgreSQL-marked tests exist at all); SPEC-001 (the implicit attribute fall-through the typed marker deliberately does not cover).
- **Tests:** style and typing conformance is enforced by the tooling itself through `task check` rather than by unit tests. The conventions that tooling cannot enforce are covered directly: the network guard's own behavior in `tests/test_network_guard.py`, and the bounded-HTTP-timeout rule — that no `urlopen` call site is left unbounded — in `tests/unit/test_http_timeouts.py`. The typed-distribution claim is split across the two places it can be observed: `TestThePackageDeclaresItselfTyped` in `tests/unit/test_package.py` pins that the marker sits inside the importable package, that it is empty rather than carrying the `partial` declaration that would mean something else entirely, and that the classifier agrees with it; the build job in `.gitlab-ci.yml` pins the half a test cannot see, that the marker is inside the built wheel and the built sdist.
