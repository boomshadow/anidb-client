...
That coupling is trusted rather than enforced, and the exposure is worth naming: nothing rejects a title that is malformed, or well-formed but carrying the wrong type. It simply produces a release-note line under the wrong heading, or under the catch-all, with no error raised anywhere along the way. The cost of the decision is therefore paid in discipline at every merge, which is the same currency the hand-curated changelog was rejected for demanding — the difference being that it is owed once per merge request rather than once per change, and that the generated list is complete whether or not the discipline holds.
